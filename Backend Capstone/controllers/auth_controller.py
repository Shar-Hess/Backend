...
def check_role(user_id, required_role='user'):
    user = db.session.query(Users).filter(Users.user_id == user_id).first()
    if not user or (required_role == 'trainer' and user.role != 'trainer'):  
        return True
...
def auth_token_add():
    ...
    if user_query:
        ...
        db.session.rollback()
        return jsonify({"message": f"Failed to create token"}), 500